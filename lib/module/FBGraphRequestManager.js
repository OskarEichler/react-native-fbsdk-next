...
function _verifyParameters(request) {
  if (request.config?.parameters) {
    for (const key of Object.keys(request.config.parameters)) {
      ...
      if (typeof param === 'object' && typeof param?.string === 'string') {
        continue;
      }
...
class FBGraphRequestManager {
  ...
  start(timeout) {
    ...
    if (this.requestBatch.length === 0) {
      throw new Error('Add at least one graph request before starting a batch.');
    }
    ...
    NativeGraphRequestManager.start(this.requestBatch.slice(), timeout || 0, callback);
  }
}
